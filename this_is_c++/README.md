...
### 상속의 장점
#### 1. 계층적 명확성
- 클래스간 상속을 통해 클래스가 계층적으로 명확해 진다
#### 2. 코드의 재사용성
- 자식 클래스는 부모 클래스로부터 상속을 받으면 부모 클래스의 멤버들을 소유하게 된다. 공통되는 기능을 부모 클래스에서 한 번만 작성해주면, 상속받아 사용할 수 있다.
#### 3. 확장성
- 기존 클래스에 없는 멤버는 새로 정의하고, 기능을 변경하거나 확장해야 하는 경우는 재정의하여 새로쿤 클래스를 정의할 수 있다.

### 상속 관계의 특성
#### 1. 상속되는 멤버와 상속되지 않는 멤버
- 부모 클래스의 생성자와 소멸자는 자식 클래스로 상속되지 않는다. 부모 클래스의 생성자는 자기 클래스의 멤버변수에 대하여만 알고 있다.
- 부모 클래스의 freind 멤버는 자식 클래스로 상속되지 않는다.

#### 2. 자식 클래스의 객체 생성 및 소멸
- 기본적으로 자식 클래스의 객체가 생성되기 전에 부모 클래스의 객체가 먼저 생성한다.
- 객체 소멸시에는 생성 순서와 역순으로 처리된다.

#### 생성자의 콜론(:)초기화
- 자식 객체 생성 순서는 부모 클래스 객체가 먼저 생성되고, 자식 클래스 객체에 나중에 생성된다. 이때 부모 클래스는 멤버 변수 초기화에 대한 문제가 발생한다. 자식 클래스에서 부모 클래스의 생성자에게 명확하게 초기값을 설정하게 함으로써 부모 클래스의 객체를 생성할 수 있게 한다.
```c++
// 부모 클래스의 생성자가 인자가 없을 때
Regular::Regular() : Employee(), salary(0.0)
{
}
// 부모 클래스의 생성자가 인자가 있을 때
Regular::Regular(char *pName, char *pAddr, double dSalary) : Employee(pName, pAddr)
{
}
```
### 부모 클래스와 자식 클래스 사이의 변환
- C++에서 지원하는 강력한 기능 중 하나가 부모 클래스 객체 포인터 또는 레퍼런스를 통해서 자식 클래스 객체에 접근이 가능하다는 것이다.
- 즉, 자식 클래스의 객체 포인터를 부모 클래스의 객체 포인터로 쉽게 변환이 가능하다는 것을 의미한다.
## ch08. 다형성(polymorphiysm)
- 가상함수란 자식 클래스에서 오버라이딩(overriding)될 것으로 예상되는 멤버함수를 의미한다. 하지만 상속에 있어서 부모 클래스의 객체를 통해서 자식 클래스에서 오버라이딩된 함수를 호출할 수 없는 문제점이 있었다.
- 가상함수는 이러한 문제점을 해결해준다. 즉, 부모 클래스의 객체를 통해서 자식 클래스에서 오버라이딩 한 함수를 호출할 수 있다는 말이다.
- 부모 클래스에 virtual 키워드를 사용하게 되면 자식 클래스에서 함수를 재정의시에 파생 클래스에 재정의된 멤버 함수가 호출된다.
- 부모에서 Paycheck()함수의 기능은 0을 반환해줄뿐 사실상 아무런 기능도 하지 않고, 자식 클래스에서 재정의하여 사용하고 있을 때, 이러한 함수를 순수 가상 함수로 지정하자(Pure Virtual Function). 가상함수 선언 맨 끝에 '0'을 대입하면 된다.
- 순수 가상 함수가 선언되어 있는 클래스는 객체를 선언할 수 없다. 객체를 통해 순수 가상 함수를 참조한다는 것은 의미없는 멤버 함수의 참조일 뿐이기 때문이다. 이러한 클래스를 '추상 클래스'라고 부른다.
### 4. 가상 소멸자
```c++
class Parent
생성자
...
void main()
{
	Parent *pParent = new Child; //Child클래스의 객체를 동적으로 생성하고, 그 포인터를 Parent 클래스의 객체포인터인 pParent에 넘겨준다.
	delete pParent; // 자식 클래스의 객체를 소멸해야 한다는 것을 인식하지 못한다. delete 연산자 자체는 자신의 오른쪽에 지정된 객체에 대한 소멸자만을 호출하는데, pParent 객체는 Parent형이므로 Parent클래스의 소멸자만 호출하게 된다.
}
======출력결과========
Parent생성자
Child생성자
Parent소멸자
```
#### 가상 소멸자(virtual destructor) 선언
- 부모 클래스의 소멸자를 가상 소멸자로 선언하면 된다.
```c++
만약 클래스에 가상함수가 있다면 그 클래스에서 파생되는 자식 클래스의 객체는 다형성을 사용하기 위해 부모 클래스의 객체 포인터를 통하여 조작될 수 있도록 new 연산자를 사용하여 동적으로 생성될 수 있을 것이다. 따라서 가상함수를 갖고 있는 클래스를 선언할 대는 가상 소멸자를 사용하는 것이 좋다.
```
