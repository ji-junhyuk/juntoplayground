...
- 패킷 유실이 발생하면 TCP와 UDP에서는 다음 차이가 발생한다
	- UDP에서는 데이터그램이 유실된다
	- UDP의 레이턴시 = 네트워크 기기의 레이턴시
	- TCP에서는 중간에 지연 시간이 발생한다
	- TCP의 레이턴시 = 네트워크 기기의 레이턴시 + (100% - 패킷 유실률) * 재전송시간
- 그러나 TCP의 레이턴시는 네트워크 게임에서 더 불쾌하게 느껴진다. 네트워크가 아주 나쁘지 않는 한 유실률은 20% 이하이지만, 재전송 대기 시간은 수백 밀리초 수준이다. TCP의 체감 레이턴시는 높은 편차때문에 더욱 불쾌감을 준다.
- 따라서 UDP는 주로 레이턴시가 민감하거나 패킷 유실이 있어도 괜찮은 곳에서 사용한다. (케릭터 이동, 기관총 난사, 음성이나 화상 데이터 전송)
- 네트워크 게임 개발에서 거의 모든 메시지 종류에는 TCP를 사용한다. 케릭터 이동 같은 몇몇 상황에서만 UDP를 사용한다. 그러나 실제 통신량의 대부분은 케릭터 이동 같은 UDP가 차지한다.
### 2.8 주로 사용하는 메시지 형식
- 게임의 메시지는 크게 텍스트나 바이너리 형식
- 바이너리 형식은 구문 분석기가 필요 없기에 성능이 더 좋다. 통신량도 더 적다. 하지만 디버깅이 까다롭다.
- 메시지에는 내용을 담는 것 뿐 아니라 어떤 것을 담고 있는지의 정보를 같이 담고 있는 형식도 있따. 흔히 메타 데이터(meta data)라고 한다.
- 메이 데이터가 있으면 두 버전이 서로 달라도 메시지 받는 쪽에서 일부 필드를 이용하여 정확한 정보를 얻을 수 있다.
- PC 온라인 게임은 게임 클라이언트를 실행하면 먼저 자동업데이트가 켜지지만, 모바일에서는 애플이나 구글의 승인과정(1~10일)을 거쳐야 한다.

### 2.9 네트워크 주소 변환(Network Address Translation, NAT)
- 다른 단말기로 전송되던 패킷의 송신자 주소나 수신자 주소가 다른 것으로 변환되는 과정을 의미한다.
- NAT 라우터는 네트워크 단자가 여러개이다.
- NAT 라우터 회선을 '인터넷 연결' 단자에 연결하고, 공유기에 연결할 각 기기는 '컴퓨터 연결' 단자에 연결한다. 주소 하나만으로 여러 기기가 인터넷을 사용할 수 있다.
- 작동 방법
	- 공유기가 패킷을 받는다. 공유기는 데이터 패킷에서 송수신자 끝점을 추출한다.
	- 공유기는 홀펀칭(포트 매핑 엔트리가 만들어지는 과정)을 통해 송신자 끝점과 수신자 끝점을 연결하는 매핑 정보를 생성해서 가지고 있다.이를 포트 매핑 엔트리(port mapping entry)라고 한다.
	- 공유기는 패킷 헤더를 변경한다. 외부 인터넷 회선으로 패킷을 보낸다. 
- 공유기는 송신자 끝점과 수신자 끝점을 이어주는 매핑 정보를 생성해서 가지고 있따. 이를 포트 매핑 엔트리(port mapping entry)라고 한다.
- 
