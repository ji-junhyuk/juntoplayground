...
void CStageNormal::run()
{
	...
	CMonster *p_monster = (CMonster *)GET_SINGLE(CObjectManager)->clone_object("troll");
	while (true)
	{
		system("cls");
		output_stage_name("Normal");
		p_player->render();
		cout << '\n';
		p_monster->render();
		cout << '\n';
			...
			case MENU_ATTACK:
				switch (battle_attack(p_player, p_monster))
				{
					...
					case BF_MONSTER_DIE:
						...
						p_monster = (CMonster *)GET_SINGLE(CObjectManager)->clone_object("troll");
						break;
				}
				break;
			case MENU_BACK:
